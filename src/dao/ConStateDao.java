...
public class ConStateDao {
	...
	public ConState getConState(int conId, int type) throws AppException {
		...
		try {
			...
			e.printStackTrace();
			throw new AppException("ConStateDao.getByConId");
		} finally {
	...
	public boolean isExist(int con_id, int type) throws AppException {
		...
		try {
			conn = JdbcUtil.getConnection();
			String sql = "select count(id) as n from t_contract_state " + "where con_id = ? and type = ? and del = 0";
			...
			e.printStackTrace();
			throw new AppException("ConStateDao.isExist");
		} finally {
			JdbcUtil.release(conn, psmt, rs);
		}
		return flag;
	...
	public boolean add(ConState conState) throws AppException {
		...
		try {
			...
			e.printStackTrace();
			throw new AppException("ConStateDao.add");
		} finally {
